		...
		for (i = 1; i < volume_height - 1; i++)
		{
			for (j = 1; j < volume_width - 1; j++)
			{
				if (voxel_pointer->extended_mask == NOMASK)
				{
					...
					voxel_pointer->reliability = H * H + V * V + N * N + D1 * D1 + D2 * D2 + D3 * D3 + D4 * D4 + D5 * D5 + D6 * D6 + D7 * D7 + D8 * D8 + D9 * D9 + D10 * D10;
				}
				voxel_pointer++;
				WVP++;
			}
...
void unwrap3D(float *wrapped_volume, float *unwrapped_volume, unsigned char *input_mask,
